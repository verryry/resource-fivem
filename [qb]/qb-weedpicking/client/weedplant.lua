...
local isPickingUp, isProcessing, isProcessing2,isProcessing3 = false, false, false
...
CreateThread(function()
	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.JointProcessing.coords, true) < 5 then
			DrawMarker(2, Config.CircleZones.JointProcessing.coords.x, Config.CircleZones.JointProcessing.coords.y, Config.CircleZones.JointProcessing.coords.z - 0.2 , 0, 0, 0, 0, 0, 0, 0.3, 0.2, 0.15, 255, 0, 0, 100, 0, 0, 0, true, 0, 0, 0)


			if not isProcessing3 and GetDistanceBetweenCoords(coords, Config.CircleZones.JointProcessing.coords, true) <1 then
				QBCore.Functions.DrawText3D(Config.CircleZones.JointProcessing.coords.x, Config.CircleZones.JointProcessing.coords.y, Config.CircleZones.JointProcessing.coords.z, 'Press ~g~[E]~w~ to Process')
			end

			if IsControlJustReleased(0, 38) and not isProcessing3 then
				local hasWeedBag = false
				local s1 = false
				local hasRollingPaper = false
				local s2 = false

				QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
					hasRollingPaper = result
					s1 = true
				end, 'rolling_paper')
				...
				if (hasWeedBag and hasRollingPaper) then
					Processjoint()
				elseif (hasRollingPaper) then
					QBCore.Functions.Notify('You dont have enough weed bag .', 'error')
				elseif (hasWeedBag) then
					QBCore.Functions.Notify('You dont have enough rolling paper.', 'error')
				else
					QBCore.Functions.Notify('You dont have enough rolling paper and weed bag.', 'error')
				end
			end
		else
			Wait(500)
...
function Processjoint()
	...
	QBCore.Functions.Progressbar("search_register", "Trying to Process..", 10000, false, true, {
		...
		function() -- success
			TriggerServerEvent('qb-weedpicking:processjoint')
			local timeLeft = Config.Delays.JointProcessing / 1000
