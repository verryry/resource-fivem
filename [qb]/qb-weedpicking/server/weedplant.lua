...
RegisterServerEvent('qb-weedpicking:processjoint', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local weed_bag = Player.Functions.GetItemByName("weed_bag")
	local rolling_paper = Player.Functions.GetItemByName("rolling_paper")

    if weed_bag ~= nil and rolling_paper ~= nil then
        if Player.Functions.RemoveItem('weed_bag', 3) and Player.Functions.RemoveItem('rolling_paper', 3) then
...
RegisterServerEvent('qb-weedpicking:cancelProcessing', function()
	...
	TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items['weed_bag'], "remove")
	TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items['rolling_paper'], "remove")
	TriggerClientEvent('QBCore:Notify', src, 'You have failed the crafting process', "error") 
